...
class Citas extends StatefulWidget {
  ...
  Citas({Key key,this.callback,this.callbackloading,this.callbackfull}) : super(key: key);
...
class _CitasState extends State<Citas>{
          ...
          child: Align(
            ...
            child: Column(
              children: <Widget>[
                ...
                Align(
                  child: Text(Strings.CuerpoTituloPaginaCitas,
                          ...
                          tabs: [
                            new Container(
                              margin: EdgeInsets.fromLTRB(
                                  0, 10, 0, 10),
                              child: new Text(Strings.Tab1Citas,
                                style: appTheme().textTheme.subhead,),
                            ), new Container(
                                margin: EdgeInsets.fromLTRB(
                                    0, 10, 0, 10),
                                child: new Text(Strings.Tab2Citas,
  ...
  Widget formulario(){
    ...
    return FutureBuilder(
        future: citasList,
        builder: (BuildContext context, AsyncSnapshot<List<CitaCompleta>> snapshot) {
                                  ...
                                  children: <Widget>[
                                    Padding(
                                      ...
                                      child: Text((snapshot.data
                                          ...
                                          .elementAt(position)
                                          .IdDoctor
                                          .Apellido, style: appTheme().textTheme
                            ...
                            children: <Widget>[
                              ...
                              Padding(
                                ...
                                child: IconButton(icon: Icon(
                                  ...
                                  final temp = DateTime.now();
                                  List<
                                      Horario> horarios = await RestDatasource()
                                    ...
                                    for (int i = 0; i < horarios.length; i++) {
                                      DateTime fechaTemp = DateTime.parse(
                                          horarios
                                              .elementAt(i)
                                              .Fecha);
                                      HorarioRango horarioid = await RestDatasource()
                                          .HorarioId(horarios
                                          .elementAt(i)
                                          .Hora);
                                      if (horarioid != null && horarios
                                          .elementAt(i)
                                          .IsAvaliable &&
                                          (fechaTemp.isAfter(temp) ||
                                              (fechaTemp.isAtSameMomentAs(
                                                  temp) && temp.hour > fechaTemp
                                                  .hour))) { //dias posteriores .. si se graba
                                        horariosId.add(horarioid);
                                        horariosAvaliable.add(
                                            horarios.elementAt(i));
                                      }
                                    }
                                  }
                                  User usuario = await RestDatasource().perfil(
                                      storageService.getEmail);
                                  List<
                                      Especialidad> especialidades = await RestDatasource()
                                      .ListaEspecialidad();
                                  int idEspecialidad = 0;
                                  for (var especialidad in especialidades) {
                                    if (especialidad.NombreEspecialidad == snapshot.data
                                        .elementAt(position)
                                        .IDEspecialidad
                                        .NombreEspecialidad) {
                          ...
                          Column(
                            ...
                            children: <Widget>[
                              Padding(
                                ...
                                child: Text((snapshot.data.elementAt(position).IDEspecialidad.NombreEspecialidad=="Odontología"?"OD. ":snapshot.data.elementAt(position).IDEspecialidad.NombreEspecialidad=="Nutrición"?"NUT. ":"PSIC. ")+snapshot.data.elementAt(position).IdDoctor.Nombre+" "+snapshot.data.elementAt(position).IdDoctor.Apellido,style: appTheme().textTheme.subhead,),//Text(this.widget.cuentas.elementAt(position).Nombre+" "+this.widget.cuentas.elementAt(position).Apellido,style: appTheme().textTheme.display4,),
                              ),
                            ],
                          ),
                        ],
                          ...
                          Column(
                            ...
                            children: <Widget>[
                              Padding(
                                ...
                                child: Text(snapshot.data.elementAt(position).Fecha.Fecha,style: appTheme().textTheme.subhead,),//Text(this.widget.cuentas.elementAt(position).Nombre+" "+this.widget.cuentas.elementAt(position).Apellido,style: appTheme().textTheme.display4,),
                              ),
                            ],
                          ),
                        ],
                          ...
                          Column(
                            ...
                            children: <Widget>[
                              Padding(
                                padding:
                                const EdgeInsets.fromLTRB(10.0, 12.0, 12.0, 3.0),
                                child: Text(snapshot.data.elementAt(position).Fecha.Hora.HorarioInicio+" "+snapshot.data.elementAt(position).Fecha.Hora.Horariofin,style: appTheme().textTheme.subhead,),//Text(this.widget.cuentas.elementAt(position).Nombre+" "+this.widget.cuentas.elementAt(position).Apellido,style: appTheme().textTheme.display4,),
                              ),
                            ],
                          ),
                        ],
          ...
          children: <Widget>[
            new FlatButton(
              ...
              onPressed: () {
                ...
                DateTime dateparse= DateTime.parse(i.Fecha.Fecha.toString()+" "+i.Fecha.Hora.HorarioInicio);
                DateTime fechaReal=dateparse.subtract(new Duration(minutes: 30));
                print(i.IdDoctor.Apellido);
                String cuerpo="Tiene cita con el doctor "+i.IdDoctor.Apellido+" en 30 minutos\n";
                String especialidad=i.IDEspecialidad.NombreEspecialidad;
                String data=i.Fecha.Hora.HorarioInicio.toString()+"-"+i.IdDoctor.Apellido+"-"+i.IDEspecialidad.NombreEspecialidad+"-"+i.Fecha.Hora.Horariofin.toString();
